    ...
    public class DbRepository<T> : Repository<T>, IRepository<T>, IOrderedQueryable<T>, IQueryable<T>, IEditable<T>, IEnumerable<T>, IOrderedQueryable, IQueryable, IQueryProvider, IEditable, IEnumerable where T : class
    {
        ...
        private class Deleteable : RouteExecuteable, IDeleteable<T>
        {
            ...
            private int Simple()
            {
                ...
                var sql = Simple(column, this, string.Empty, parameters);
            ...
            private int Complex()
            {
                ...
                var sql = Complex(columns, this, string.Empty, parameters);

                return Editable.Excute(sql, parameters);
            }

            private string Simple(KeyValuePair<string, string> column, IEnumerable<T> collect, string group, Dictionary<string, object> parameters)
            {
                ...
                var list = collect.Select((item, index) =>
                 {
                     ...
                     var parameterKey = Settings.ParamterName($"{name}_{group}_{index}");
            ...
            private string Complex(List<KeyValuePair<string, string>> columns, IEnumerable<T> collect, string group, Dictionary<string, object> parameters)
            {
                var sb = new StringBuilder();
                ...
                sb.Append("DELETE FROM ")
                   ...
                   .Append(string.Join(" OR ", collect.Select((item, index) =>
                   {
                       ...
                       return string.Concat("(", string.Join(" AND ", columns.Select(kv =>
                       {
                           ...
                           var parameterKey = Settings.ParamterName($"{kv.Key.ToUrlCase()}_{group}_{index}");
        ...
        private class Updateable : RouteExecuteable, IUpdateable<T>
        {
            ...
            public override int Execute()
            {
                ...
                this.ForEach((item, index) =>
                {
                    ...
                    .Select(kv =>
                    {
                        string parameterKey = Settings.ParamterName($"{kv.Key.ToUrlCase()}_{index}");
                    ...
                    sb.Append("UPDATE ")
                        ...
                        {
                            string parameterKey =
                        typeRegions.Tokens.ContainsKey(kv.Key) ?
                        Settings.ParamterName($"__token_{kv.Key.ToUrlCase()}_{index}")
                        :
