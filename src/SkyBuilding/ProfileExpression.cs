...
namespace SkyBuilding
{
    ...
    public abstract class ProfileExpression<T> : IProfileExpression, IProfile where T : ProfileExpression<T>
    {
        ...
        /// <returns></returns>
        public Func<object, TResult> Create<TResult>(Type sourceType) => Nested<TResult>.Create((T)this, sourceType ?? throw new ArgumentNullException(nameof(sourceType)));
        ...
        /// <param name="conversionType">目标数据类型</param>
        /// <returns></returns>
        protected Func<object, TResult> ByObject<TResult>(Type sourceType, Type conversionType)
        {
            if (conversionType.IsGenericType)
            {
                if (conversionType.IsInterface)
                {
                    if (conversionType.GetGenericTypeDefinition() == typeof(ICollection<>))
                        return ByObjectToCollectionG<TResult>(sourceType, conversionType, conversionType.GetGenericArguments().First());

                    if (conversionType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
                        return ByObjectToEnumarableG<TResult>(sourceType, conversionType, conversionType.GetGenericArguments().First());
                }
                ...
                if (conversionType.IsClass && !conversionType.IsAbstract)
                {
                    ...
                    if (types.Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(ICollection<>)))
                        return ByObjectToCollectionG<TResult>(sourceType, conversionType, types.First(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(ICollection<>)).GetGenericArguments().First());
                }
            }
        ...
        /// <param name="genericType">泛型约束</param>
        /// <returns></returns>
        protected virtual Func<object, TResult> ByObjectToEnumarableG<TResult>(Type sourceType, Type conversionType, Type genericType) => throw new InvalidCastException();
        ...
        /// <param name="genericType">泛型约束</param>
        /// <returns></returns>
        protected virtual Func<object, TResult> ByObjectToCollectionG<TResult>(Type sourceType, Type conversionType, Type genericType) => throw new InvalidCastException();
        ...
        protected virtual Func<object, TResult> ByEnumarable<TResult>(Type sourceType, Type conversionType)
        {
            if (conversionType.IsGenericType)
            {
                ...
                {
                    if (conversionType.GetGenericTypeDefinition() == typeof(ICollection<>))
                        return ByEnumarableToCollectionG<TResult>(sourceType, conversionType, conversionType.GetGenericArguments().First());

                    if (conversionType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
                        return ByEnumarableToEnumarableG<TResult>(sourceType, conversionType, conversionType.GetGenericArguments().First());
                }
                ...
                if (conversionType.IsClass && !conversionType.IsAbstract)
                {
                    ...
                    if (types.Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(ICollection<>)))
                        return ByEnumarableToCollectionG<TResult>(sourceType, conversionType, types.First(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(ICollection<>)).GetGenericArguments().First());
                }
            }
        ...
        /// <param name="genericType">泛型约束</param>
        /// <returns></returns>
        protected virtual Func<object, TResult> ByEnumarableToEnumarableG<TResult>(Type sourceType, Type conversionType, Type genericType) => throw new InvalidCastException();
        ...
        /// <param name="genericType">泛型约束</param>
        /// <returns></returns>
        protected virtual Func<object, TResult> ByEnumarableToCollectionG<TResult>(Type sourceType, Type conversionType, Type genericType) => throw new InvalidCastException();
